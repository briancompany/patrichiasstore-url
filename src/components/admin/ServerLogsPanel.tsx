...
import { AlertTriangle, Activity, Clock, RefreshCw, ShieldAlert } from 'lucide-react';
...
export function ServerLogsPanel() {
  ...
  return <div className="space-y-4">
    ...
    ].map(([label, value, Icon]) => <Card key={String(label)}><CardContent className="p-4 flex justify-between"><div><p className="text-sm text-muted-foreground">{String(label)}</p><p className="text-2xl font-bold">{String(value)}</p></div><Icon className="h-5 w-5 text-primary" /></CardContent></Card>)}</div>
