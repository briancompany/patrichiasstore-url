...
export function ShopPriceChart() {
  ...
  useEffect(() => {
    // Product data is the primary shop content. Load this secondary content
    // after the browser has had a chance to render the product grid.
    const load = () => {
      supabase
        .from('store_content')
        .select('price_chart_url, shop_description')
        .limit(1)
        .maybeSingle()
        .then(({ data }) => {
  ...
  return (
    <>
            ...
            <CardContent className="p-0">
              <div className="relative group">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-4 flex justify-between items-end opacity-0 group-hover:opacity-100 transition-opacity">
                  <Button variant="secondary" size="sm" onClick={() => setZoomed(true)} className="gap-2">
      ...
      {zoomed && content.price_chart_url && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-start justify-center overflow-auto p-4" onClick={() => setZoomed(false)}>
          <div className="relative max-w-4xl w-full" onClick={(e) => e.stopPropagation()}>
            <Button variant="ghost" size="icon" className="absolute -top-2 -right-2 z-10 bg-white/20 hover:bg-white/40 text-white rounded-full" onClick={() => setZoomed(false)}>
              <X className="h-5 w-5" />
            </Button>
            <img src={content.price_chart_url} alt="Price Chart - Full Size" className="w-full h-auto rounded-lg" />
